...
La {numref}`kellipse-ref` présente un  exemple simple du principe développé ci-après. 

```{figure} ./images/Kellipse.png
:name: kellipse-ref
```
...
````{tabbed} Noyau polynomial

Considérons $K(\mathbf x_i, \mathbf x_j) = \left (\mathbf x_i^T \mathbf x_j +\theta\right )^p$ En développant on obtient  ${d + p - 1 \choose p}$ termes, chacun d'entre eux étant un polynôme de degré variable des vecteurs d'entrée. $K$ peut donc être vu comme le produit scalaire de deux vecteurs $\mathbf z$ de très grande taille, d'autant plus importante que $p$ est grand.

````
````{tabbed}  Noyau  gaussien
