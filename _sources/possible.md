<div class="admonition note" name="html-admonition" style="background: lightcoral; padding: 10px">
...
En apprentissage, l'inconnue n'est pas un réel ($\mu$), mais une fonction $f:X\rightarrow Y$. Le candidat est lui aussi une fonction $h$ appartenant à un ensemble $\mathcal{F}$ .Dans la {numref}`fhe-ref`, la figure de gauche présente un exemple de fonction $f$ à atteindre, et d'une hypothèse $h$ (classification binaire). La fonction d'erreur (à droite) donne le lieu des points $\mathbf x$ tels que $h(\mathbf x)=f(\mathbf x)$ (bleu), $h(\mathbf x)\neq f(\mathbf x)$ (rouge). L'erreur de $h$ est donc $E(h)=P_\mathbf x(h(\mathbf x)\neq f(\mathbf x))$ (l'aire de la région rouge).
...
Soit $Z=\{\mathbf x_i\in X,i\in [\![1\cdots n]\!]\}$ un ensemble de $n$ points de $X$.  On note 
...
Soit $Z=\{\mathbf x_i\in X,i\in [\![1\cdots n]\!]\}$ un ensemble de $n$ points de $X$ et $\mathcal{F}$ un ensemble de fonctions. On dit que $\mathcal{F}$ pulvérise $Z$
...
$$|E_t(g)-E_g(g)|\leq\sqrt{\frac{1}{2n}log\frac{2\Delta_\mathcal{F}(n)}{P}}$$ et :
